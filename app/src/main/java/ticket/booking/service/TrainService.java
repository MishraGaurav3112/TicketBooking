...
public class TrainService {
    ...
    public Train findTrainByTrainNo(int trainNo) {
        List<Train> allTrains = getAllTrains(); // This should read or access your train data

        for (Train train : allTrains) {
            try {
                if (Integer.parseInt(train.getTrainNo()) == trainNo) {
                    return train;
                }
            } catch (NumberFormatException e) {
                // skip if trainNo is not parsable
            }
        }
